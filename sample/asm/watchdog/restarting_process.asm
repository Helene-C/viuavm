...
.function: watchdog_process/1
    ...
    remove (.name: %iota exception) local %1 local (string %exception local "exception") local
    remove (.name: %iota aborted_function) local %1 local (string %aborted_function local "function") local
    remove (.name: %iota parameters) local %1 local (string %parameters local "parameters") local
    ...
    copy (.name: %iota i) local *(vat %i local %parameters local (integer %iota local 1) local) local
    frame ^[(param %0 *(vat %message local %parameters local (integer %iota local 0) local) local) (param %1 (iinc %i local) local)]
...
.function: formatting/2
    ...
    string (.name: %iota format_string) local "#{0} / #{1}"
    frame ^[(param %0 %format_string local) (param %1 %divide_what local) (param %2 %divide_by local)]
    move %0 local (msg %iota local format/) local
